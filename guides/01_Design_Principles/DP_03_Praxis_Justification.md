...
### Example 2: Communication Protocol Interpretation (prx_)

**Scenario**: Feature interprets communication protocol frames where the protocol byte layout IS the domain model.

- **Domain judgment**: "If frame ID 0x3A byte[2] bit 5 is set, ambient light mode is active"
- **External dependency**: Communication middleware provides `Comm_Frame_t` with byte array
...
```
prx_comm_ambient.c  →  "interpret Comm_Frame_t byte[2] bit 5 as ambient mode"
```
